...
import 'carib_login.dart';
import 'terms_of_service_checkbox.dart';
  ...
  void initState() {
    ...
    if (widget.userType == LoginUserType.intlPhone) {
      _phoneNumberInitialValue = PhoneNumber(
        ...
        phoneNumber:
            null, // Initialize with null if no phone number is provided
      );
      ...
      if (widget.controller?.value.text != null) {
        try {
          ...
          if (parsed.isValid()) {
            _phoneNumberInitialValue = PhoneNumber(
              phoneNumber: parsed.nsn, // National significant number
